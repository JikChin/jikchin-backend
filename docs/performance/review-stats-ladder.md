...
희생을 드러내기 위한 탐색 실행(같은 피리뷰어 1명에게 집중, 사다리 비교값 아님): 50 rps × 30s에서 p50 9.0ms / p95 15.4ms, 락 대기 0건. 200 rps × 15s(3,000건)에서 p50 3.0ms / p95 4.1ms, `Innodb_row_lock_waits` 4건, 누적 대기 16ms, 최대 9ms. 집계 행은 원본과 1,003,000건으로 일치했다. 즉 직렬화는 실재하지만 이 규모에서는 요청 1,000건당 1건 수준, 최대 한 자리 ms다. 리뷰 한 건이 수 ms로 끝나므로 락 점유 시간이 짧아, 한 피리뷰어에게 초당 수백 건이 몰리는 상황이 아니면 병목이 되지 않는다. (200 rps 실행의 실패 1건은 3,001번째 요청이 픽스처 글 3,000건을 넘어선 것으로 서버 오류가 아니다.)
...
| 쓰기 p95·처리량 | 1·2단계에서 악화, 3단계에서 회복 (실측은 1·2단계 모두 개선. `updateMannerScore`의 O(N) AVG가 함께 사라졌기 때문, 각 단계 절 참고) | `http_req_duration` p95, `http_reqs`, `Innodb_row_lock_waits` |
...
| 2. 집계 테이블(동기) | 13 / 17 / 21 | 2.02 | 23 / 28 / 32 | 2.02 | type = const | 쓰기 락 직렬화(200 rps에서 3,000건 중 4건, 최대 9ms), 집계 테이블·백필·코드 복잡도 |
| 3. 마이크로 배치 | | | | | | 정합성 지연 |
...
| 2. 집계 테이블(동기) | 1 | 14.8 / 18.7 / 21.0 | 2.01 | 24.9 / 30.8 / 35.1 | 2.03 | 0% | 0 | 0.003 |
| 2. 집계 테이블(동기) | 2 | 13.5 / 16.7 / 20.2 | 2.03 | 22.9 / 27.3 / 29.7 | 2.01 | 0% | 0 | 0.003 |
| 2. 집계 테이블(동기) | 3 | 12.0 / 14.8 / 22.4 | 2.03 | 21.9 / 26.2 / 31.1 | 2.01 | 0% | 0 | 0.003 |
